...
const Aside: React.FC = () => {
  ...
  const { isLoading, data, isError } = useSelector(
    (state: RootState) => state.monthBooks
  );
  ...
  return (
    <aside className="text-center w-full  lg:w-[280px] p-4 flex flex-col items-center md:items-start  gap-5">
      <h1 className="md:w-[270px] font-bold italic text-2xl lg:text-4xl text-customTextColour">
        Best this Month
      </h1>
      <div className=" bg-slate-50 rounded-lg">
        <div className="flex flex-col gap-5 items-center border-gray-200 p-4 rounded-md">
          {asideBooks.slice(0, 4).map((book: Book) => (
